...
The \texttt{OutboxEventPublisherDecorator} whitelists only the three outbound cross-context event types (\texttt{OrderPlacedEvent}, \texttt{OrderConfirmedEvent}, \texttt{OrderCompensatedEvent}). All other events (including Brevo/Omnisend/nopCommerce-internal events) pass through unchanged. This avoids breaking existing plugins but means adding a new cross-context event requires a code change to the whitelist. We consider this deliberate friction: cross-context events represent API surface between bounded contexts and should not be added carelessly.
