...
\begin{itemize}
    \item nopCommerce storefront: \texttt{http://localhost:5000}
    \item nopCommerce admin: \texttt{http://localhost:5000/admin}
...
docker compose stop inventory-sync
    \end{lstlisting}
    \item Place two or three orders via the storefront. Each should receive an order confirmation immediately.
    \item In the SQL client, observe \texttt{OutboxMessage}: rows accumulate with \texttt{DispatchedOnUtc IS NULL} (for the \texttt{StockReservedEvent} / \texttt{OrderPlacedEvent} rows pending consumption) or \texttt{DispatchedOnUtc} is set (the outbox dispatched to RabbitMQ, but the Inventory service is not consuming).
...
\begin{enumerate}
    ...
    \begin{lstlisting}[language=bash]
docker compose start inventory-sync
    \end{lstlisting}
...
\begin{enumerate}
    \item Configure the Inventory Sync Service (or its OpenBoxes stub) to return a rejection response for the next reservation request. (See the WireMock / stub configuration in \texttt{docker/wiremock/mappings/}.)
